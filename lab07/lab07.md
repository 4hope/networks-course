...
1. Какой IP-адрес и номер порта TCP использует ваш компьютер (отправитель), передающий
   ...
   - IP-адрес: 192.168.0.101, TCP-порт: 51650
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема TCP-сегментов этого соединения?
   ...
   - серверный порт: 80, клиентский порт: 51650
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления TCP-соединения между компьютером клиента и сервером gaia.cs.umass.edu? Как определяется, что это именно SYN-сегмент?
   - Относительный порядковый номер: Seq = 0, абсолютный: Seq = 3625160072
   - Это именно SYN-сегмент, потому что в поле TCP Flags установлен флаг: Flags: 0x002 (SYN)
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu на компьютер клиента в ответ на SYN-сегмент? Какое значение хранится в поле подтверждения в SYNACK-сегменте? Как сервер gaia.cs.umass.edu определил это значение? Как определяется, что это именно SYNACK-сегмент?
   - Относительный порядковый номер сервера: Seq = 0, абсолютный: Seq = 1078300629
   - Поле подтверждения: относительное значение: Ack = 1, абсолютное: Ack = 3625160073
   - сервер подтверждает клиентский SYN, у которого начальный sequence number был 0. SYN занимает один номер последовательности, поэтому сервер отвечает Ack = 0 + 1 = 1
   - Это именно SYNACK-сегмент, потому что у него в поле флагов указано: Flags: 0x012 (SYN, ACK)
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - Относительный порядковый номер: Seq = 1, абсолютный: Seq = 3625160073
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - Порядковые номера первых шести TCP-сегментов: первый, содержащий команду POST, имеет Seq = 1; второй - Seq = 1461; третий - Seq = 2921; четвёртый - Seq = 4381; пятый - Seq = 5841; шестой - Seq = 7301.
   - Все сегменты были отправлены в 27.860023с от начала захвата.
   - Все ACK-пакеты для этих сегментов были получены в 28.009755с.
   - Разница между временем отправки каждого TCP-сегмента и временем получения подтверждения для каждого из первых шести сегментов равна 28.009755 − 27.860023 = 0.149732с. Тогда RTT для каждого сегмента равен 0.149732с.
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - Пропускная способность TCP-соединения при передаче POST-запроса равна примерно 248562 байт/с. 
   - Это значение получено делением количества переданных TCP-данных 152888 байт на время между отправкой первого POST-сегмента и получением ACK, подтверждающего весь POST-запрос: 0.615090 с
...
#### Решение
Bandwidth–delay product: $BDP = R * RTT = 10^9 * 0.03 = 3 * 10^7$ бит.
Окно из $n$ пакетов содержит $n * 12000$ бит.
Коэффициент использования канала: $U = \frac{n * 12000}{3 * 10^7}$.
Условие $U > 0.98$: $\frac{n * 12000}{3 * 10^7} > 0.98$.
Отсюда $n > \frac{0.98 * 3 * 10^7}{12000} = \frac{29400000}{12000} = 2450$.
Так как $n$ целое и требуется >98%, минимальное значение $n = 2451$.
