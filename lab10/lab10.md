...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   ...
   - Общая длина IP-дейтаграммы: 56 байт, значит полезная нагрузка IP-дейтаграммы равна 56 - 20 = 36 байт.
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Меняются поля Identification, Time to Live (TTL) и Header Checksum.
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Не меняются: Source IP, Destination IP, Protocol, Header Length, Total Length, Flags, Fragment Offset.
      - Должны оставаться неизменными: Source Address, Destination Address, Protocol, Header Length, Total Length.
      - Должны меняться: Identification, Header Checksum, TTL.
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Значение Identification увеличивается с каждым последующим ICMP Echo Request.
      - Закономерность: для последовательных исходящих дейтаграмм значение увеличивается на 1. (например, в пакетах №305, №306 значения Identification равны 21836, 21837)
5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   (выберите какой-либо один пакет ICMP)?
   - Для пакета №306: Identification = 21837.
   - Для пакета №306: TTL = 1.
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Поле Identification не остаётся неизменным.
   - Поле TTL же остаётся одинаковым: TTL = 64. Например, ближайший маршрутизатор 192.168.0.1 присылает ICMP Time-to-live exceeded, и в этих пакетах TTL равен 64.
7. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
   ...
   - Я взяла пакет №307.
   - В этом пакете: Identification = 3116, TTL = 64.
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Да было фрагментировано. Было создано 3 IP-фрагмента.
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - В разных фрагментах изменяются Total Length, Flags, Fragment Offset и Header Checksum. Поле Identification у фрагментов одного исходного пакета остается одинаковым.
