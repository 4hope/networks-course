...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - В DHCP Request и DHCP ACK используется одинаковый Transaction-ID: 0xbd5caa7b.
   - Поле Transaction-ID нужно, чтобы клиент мог связать DHCP-ответ сервера с конкретным DHCP-запросом. То есть по этому полю понятно, что данный ACK является ответом именно на этот Request.
4. Хост использует DHCP-протокол, в частности, для получения IP-адреса. Но IP-адрес хоста
   ...
   - Если IP-адрес ещё не установлен, клиент обычно использует исходный IP-адрес 0.0.0.0, а конечный IP-адрес 255.255.255.255.
   - У меня такой обмен Request/ACK: DHCP Request отправлен с 192.168.0.101 на 192.168.0.1, а DHCP ACK отправлен с 192.168.0.1 на 255.255.255.255.
5. Каков IP-адрес вашего DHCP-сервера?
   - 192.168.0.1
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   вашем эксперименте?
   - Срок аренды IP-адреса показывает, на какое время DHCP-сервер выдаёт клиенту IP-адрес. После истечения этого времени клиент должен обновить аренду или получить новый адрес.
   - У меня срок аренды равен 86400 секунд.
...
Эффективность дискретного ALOHA равна $S(p) = Np(1 - p)^{(N - 1)}$.
Нужно найти такое p, при котором S(p) максимально. Так как N - это просто постоянное число, можно максимизировать только часть $p(1 - p)^{(N - 1)}$.
Берём производную: $f'(p) = (1 - p)^{(N - 2)}(1 - Np)$. Чтобы найти максимум, приравниваем производную к нулю: $1 - Np = 0$. Отсюда получаем: $p = 1/N$.
Теперь подставим это значение в исходную формулу эффективности: $S = N * (1/N) * (1 - 1/N)^{(N - 1)}$.
После сокращения получаем: $S = (1 - 1/N)^{(N - 1)}$.
Когда N стремится к бесконечности, это выражение стремится к $1/e$.
То есть максимальная эффективность дискретного ALOHA при большом числе узлов равна $1/e ≈ 0.368$.
